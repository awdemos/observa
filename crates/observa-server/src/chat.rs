...
pub async fn create_session(
    ...
    match state.chat_store.create_session().await {
        Ok((session_id, owner_token)) => {
            Json(SessionResponse { session_id, owner_token }).into_response()
        }
...
pub async fn ask(
    ...
    match ask_core(&state, req.session_id, &req.owner_token, &req.message, req.system_prompt.as_deref()).await {
        Ok(reply) => {
...
pub async fn stream(
    ...
    match do_stream(state, query.session_id, &query.owner_token, query.message, query.system_prompt).await {
